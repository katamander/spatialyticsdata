...
rcl <- cbind(-10000, 0, NA)

# Reclassify 
?reclassify()
elevation_reclass <- classify(elevation, rcl = rcl)
...
bb <- st_make_grid(mounds, n = 1)

# plot everything together
plot(elevation_reclass);
plot(mounds$geometry, add = T, col = "white"); 
...
plot(st_geometry(survey_big), add = T, border = "white")
...
mounds$elev  <- terra::extract(elevation, mounds)[,2]
...
elev
prominence <- rast("../data/prominence1500m.tif")
prominence

# Plot prominence on top of elevation
plot(elevation_reclass); 
plot(prominence, add = T)

# Align the extent of the two rasters with crop()
elev <- crop(elevation_reclass, prominence)

# Do the overlay using the boolean operators.
prom_el_overlay <- elev>450 & elev<600 & prominence >60

hist(as.numeric(values(prom_el_overlay)))
...
```{r prom-mnd-ex}
library(mapview)
mapview(mounds, zcol = "elev")

mounds$prom <- terra::extract(prominence, mounds)[,2]
mounds$overlay<- terra::extract(prom_el_overlay, mounds)[,2]

mounds %>% 
  filter(overlay == 1) %>% 
  arrange(desc(prom)) %>% 
  select(TRAP_Code, prom) %>% 
  slice(1:25) %>% 
  mapview(zcol = "prom")
